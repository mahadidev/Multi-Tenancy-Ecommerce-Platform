import useTrendingProducts from '@seller/hooks/useTrendingProducts';
...
const TrendingProducts: React.FC<TrendingProductsProps> = ({ className = "" }) => {
	...
	const { products, isLoading, isError, error, refetch } = useTrendingProducts({
	...
	const currentFilterOption =
		...
		filterOptions[0];
	const currentTimeRangeOption =
		timeRangeOptions.find((item) => item.value === currentTimeRange) ||
		timeRangeOptions[1];
