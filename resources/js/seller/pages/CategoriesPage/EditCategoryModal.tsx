						...
						<div className="flex flex-col gap-2">
							...
							<div>
								<Select
									...
									helperText={
										formErrors['parent_id'] ? formErrors['parent_id'][0] : false
									}
								...
								>
									...
									{productCategories.filter((filterCategory) => filterCategory.id !== formState.id).map((category: CategoryType) => (
										<option
											value={category.id}
											key={category.id}
											selected={category.id === props.category.has_parent?.id}
										>
											{category.name}
										</option>
									))}
