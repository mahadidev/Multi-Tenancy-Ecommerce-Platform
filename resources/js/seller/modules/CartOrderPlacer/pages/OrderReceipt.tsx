...
const OrderReceipt: React.FC<OrderReceiptProps> = ({order}) => {
    ...
    const totalDiscount = order?.items?.reduce((sum, item) => sum + (item.price - (item.discount_price ?? 0) || 0), 0) || 0;
    ...
    const totalShipping =
			order?.items?.reduce((sum, item) => sum + ( 0), 0) || 0;
					...
					<table className="w-full mb-3">
						...
						<tbody>
							{order?.items?.map((item) => {
								...
								const discountUnitPrice = item.discount_price
