import {
    setInSidebarCollapsed,
    setInsideIsOpenMobile,
    toggleInsideIsOpenMobile,
} from "@seller-panel/store/slices/uiSlice";
import { useAppDispatch, useAppSelector } from "@seller-panel/store/store";
import type { PropsWithChildren } from "react";
import { createContext } from "react";

interface SidebarContextProps {
    desktop: {
        isCollapsed: boolean;
        setCollapsed(value: boolean): void;
        toggle(): void;
    };
    mobile: {
        isOpen: boolean;
        close(): void;
        toggle(): void;
    };
}

const SidebarContext = createContext<SidebarContextProps | null>(null);

export function InSidebarProvider({
    children,
}: PropsWithChildren<{ initialCollapsed: boolean }>) {
    const { isOpenMobile } = useAppSelector(
        (state) => state.ui.sidebar.mobile
    );
    const { isCollapsed } = useAppSelector(
        (state) => state.ui.sidebar.desktop
    );
    const dispatch = useAppDispatch();

    function handleSetCollapsed(value: boolean) {
        dispatch(setInSidebarCollapsed(value));
    }

    function handleIsOpenMobile(value: boolean) {
        dispatch(setInsideIsOpenMobile(value));
    }

    function handletoggleInsideIsOpenMobile() {
        dispatch(toggleInsideIsOpenMobile());
    }

    return (
        <SidebarContext.Provider
            value={{
                desktop: {
                    isCollapsed,
                    setCollapsed: handleSetCollapsed,
                    toggle: () => handleSetCollapsed(!isCollapsed),
                },
                mobile: {
                    isOpen: isOpenMobile,
                    close: () => handleIsOpenMobile(false),
                    toggle: () => handletoggleInsideIsOpenMobile,
                },
            }}
        >
            {children}
        </SidebarContext.Provider>
    );
}
